...
export default class App extends Component {
  ...
  renderSearchPodcasts = () => {
    ...
    const subscriptionIds = subscriptions.length
      ? subscriptions.map(podcast => podcast.collectionId)
      : [];
      ...
      >
        ...
        >
          {podcast
            ...
            : subscriptions.map(podcast => this.renderlistenPodcast(podcast))}
        </ScrollView>
        {this.renderButtons()}
      </View>
    );
  ...
  renderPodcastTrack = track => {
    const guid = Array.prototype.slice.call(track.getElementsByTagName("guid"));
